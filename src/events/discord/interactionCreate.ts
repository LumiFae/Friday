...
export default async function (client: Client) {
    client.on('interactionCreate', async (interaction) => {
        ...
        const serverLocale = new Locales(await getLocale(interaction.guildId, true));
        const userLocale = new Locales(await getLocale(interaction.user.id, false));
        try {
            await (command.run as (interaction: Interaction, serverLocale: Locales, userLocale: Locales) => unknown)(
                interaction,
                serverLocale,
                userLocale,
            );
