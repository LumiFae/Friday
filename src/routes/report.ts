import {CategoryChannel, Client, ChannelType, OverwriteResolvable} from 'discord.js';
...
export default function (app: Express, client: Client) {
    app.get('/report', async (req, res) => {
        ...
        const server = (await db.select().from(servers).where(eq(servers.token, auth)).execute().catch(() => [null]))[0];
        ...
        if(!(category instanceof CategoryChannel)) return res.status(400).send('Invalid category');

        let discordUserId = await getUser(body.reportedId, true);
        if(discordUserId && !(await new DiscordFetch(client).member(server.id, discordUserId.id))) discordUserId = null;
        const ticketInfo = (await db.insert(tickets).values({ created_by: discordUserId?.id || null, server: server.id }).returning().execute().catch(() => [null]))[0];
        if(!ticketInfo) return res.status(500).send('Failed to create ticket');

        const permissionOverwrites: OverwriteResolvable[] = [{
                id: guild.roles.everyone.id,
                allow: ['EmbedLinks', 'AttachFiles', 'ReadMessageHistory', 'SendMessages'],
                deny: 'ViewChannel'
            },
            {
        ...
        if(server.mod_role) permissionOverwrites.push({
            id: server.mod_role,
            allow: 'ViewChannel'
        });
        if(discordUserId) permissionOverwrites.push({
            id: discordUserId.id,
            allow: 'ViewChannel'
        });
        ...
        const embed = embed_()
            .setTitle(serverLocale.get((lang) => lang.ticket.embeds.title));
        const embedFields: { name: string; value: string }[] = []
        const embedFieldNames = serverLocale.getObject((lang) => lang.ticket.embeds.field_names);
        ...
        await channel.send({
            content: discordUserId ? server.message ? replacement(server.message,`<@${discordUserId}>`) : replacement(serverLocale.get((lang) => lang.ticket.default_message), `<@${discordUserId}>`) : serverLocale.get((lang) => lang.ticket.default_message_no_user),
            embeds: [embed]
        })
    });
}
