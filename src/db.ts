...
export async function setSteamData(steamid: string | null, clientid: string) {
    await db.insert(users).values({ id: clientid, steamid }).onConflictDoUpdate({
        target: users.id,
        set: { steamid },
    })
}
...
export async function getUser(id: string, steam = false) {
    ...
    return (await db.select().from(users).where(whereQuery).execute().catch(() => [null]))[0];
}

export async function getServer(id: string) {
    return (await db.select().from(servers).where(eq(servers.id, id)).execute().catch(() => [null]))[0];
}
