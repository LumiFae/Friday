...
export default {
    ...
    run: async (interaction, serverLocale, userLocale) => {
        ...
        const token = (await db.insert(servers).values({ id: interaction.guildId, token: generateRandomString(20) }).onConflictDoUpdate({
            target: servers.id,
            set: { token: generateRandomString(20) },
        }).returning({ token: servers.token }).execute())[0].token as string;
        const embed = embed_()
            .setTitle(userLocale.get((lang) => lang.setup.embeds.title))
            .setDescription(replacement(userLocale.get((lang) => lang.setup.embeds.description), token))
